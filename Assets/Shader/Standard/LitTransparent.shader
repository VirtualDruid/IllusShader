Shader "Celluloid/LitTransparent"
{
    Properties
    {
        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                float intense(float3 color) {
                    return pow(dot(color, color) / 3, 0.5);
                }
                ...
                void PixelShaderEntry(VertexShaderOutput i, in bool isFrontFace: SV_IsFrontFace, out float4 forwardOutput : SV_Target0) {
                    ...
                    forwardOutput = float4(diffuse + specular + clearcoat + transmission + envSpecular + minput._Emissive, 
                        saturate(minput._Alpha + intense(specular + clearcoat + envSpecular + minput._Emissive) * (1 - minput._Alpha)));
                    //float3 a, b, c, d, e;
                    //float4 _dbg;
                    //Forward(linput, minput, sinput, a, b, c, d, e);
                    //forwardOutput = float4(a + b + c + d + e , saturate(minput._Alpha + intense(b + c + e) * (1 - minput._Alpha)));
